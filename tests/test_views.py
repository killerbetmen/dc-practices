...
def test_update_message():
    response = requests.put(f"http://127.0.0.1:8000/update_message/1",
                            json={"Message": "update"},
                            auth=HTTPBasicAuth(username, password),
                            )
    response_1 = client.get(f"/messages/1")
    assert response.status_code == 200
...
def test_delete_message_uu():
    response = requests.delete(f'http://127.0.0.1:8000/delete_message/1',
                               auth=HTTPBasicAuth('BadUser', 'badpassword')
                               )
    assert response.status_code == 401
